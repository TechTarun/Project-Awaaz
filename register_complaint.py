...
def get_folder_name(category_name):
	folder_name = '_'.join(category_name.lower().split())
	return folder_name

def make_required_folders_if_not_exists():
	if not os.path.isdir('tmp'):
		os.mkdir('tmp')
	for category in CATEGORIES:
		directory = get_folder_name(category)
		if not os.path.isdir(directory):
			os.mkdir(directory)

def execute():
	make_required_folders_if_not_exists()
	...
	filename = "tmp/" + ac_num + "_" + str(t.year) + "_" + str(t.month) + "_" + str(t.day) + "_" + str(t.hour) + "_" + str(t.minute) + ".txt"
...
def sort_into_folders(complaint, keywords_dict, filename, folders):
    ...
    newfilename = get_folder_name(folders[folder_num]) + "/" + filename.split('/')[1]
    shutil.move(filename, newfilename)


#main
CATEGORIES = ["ATM and e-facilities", "Personnel", "Loan and lending", "Cashier"]
execute()
